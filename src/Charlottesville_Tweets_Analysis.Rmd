---
title: 'Twitter as Public Space: an exploratory analysis of #Charlottesville on Twitter'
author: "James Alster"
date: "09/06/2020"
output:  
  word_document:
    reference_docx: ../markdown_resources/Word-Styles.docx
bibliography: [../markdown_resources/articles.bib,./markdown_resources/software.bib]
...
``` {r tweets_over_time, fig.asp = 5/9, fig.cap = "Figure 1: Distribution of tweets by datetime.\nTimes are in EST, the local time in Charlottesville and of Trump’s press conference."}
...
The timezone data provided in the data set (Table 1) shows that the public debate over Trump’s comments on Charlottesville was literally worldwide. The majority of participants hid their data from Twitter (a ‘null’ value) or came from the USA; [^2] but 17% came from elsewhere, with small numbers in a wide variety of African and Asian countries. Those probably include many expatriates, although `r round(( 1 - (nrow(tweets) / nrow(raw_tweets))) * 100, 1)`% of the dataset is not in English. Although some people may have had multiple accounts, and some accounts are automated, the public debate around Charlottesville contained in this dataset alone actively engaged about a hundred thousand people, over ten thousand of whom were abroad. The heavily mediatised events at Charlottesville became a focus for interaction on a global scale, almost completely divorced from actual presence at the event or any concrete personal impact (Goldfarb 2018).  

[^2]: While some US timezones might represent other American locations, Twitter designates specifically Canadian and South American timezones, which appear in the dataset.

The dataset is an opportunity to study the structure and characteristics of this massive, global public debate: how did people conceptualise their role in public space? How did they position, associate or contrast other actors? What social ties and oppositions did they construct or invoke? The analysis that follows is only exploratory, and seeks to provide indications of how these questions might be more fully answered.

# Initial Analysis

The dataset contains the full text of `r nrow(raw_tweets)` tweets, of which `r round(nrow(tweets) / nrow(raw_tweets) * 100, 1)`% are in English. [^3]  They come with the time the tweet was created, as well as URLs to picture or content, which I have not recuperated here. None of the tweets are retweets, which will exclude most automated accounts. `r sum(!is.na(raw_tweets$in_reply_to_status_id))` are replies, of which only `r with(raw_tweets, sum(id %in% in_reply_to_status_id))` are replies to other tweets in the data set: the collection of tweets by term rather than by snowballing has made it impossible to study specific user interactions.

[^3]: All text analysis below is of english tweets only.
...
``` {r numeric_user_vars, fig.asp = 5/9, fig.cap = "Figure 2: Distribution of users by number of followers"}
...
As for the number of followers, which influences the visibility of each tweet by a user, this follows a  log-normal distribution, shown in Figure 2. A few users have followers over three orders of magnitude higher than the mean, which is 11505 followers. Figure 2 also compares the users in the dataset with my own random sample of 9000 users, [^4] to indicate the typicality of the composition of the tweeting public in terms of followers. While the distributions are globally similar, the users in the Charlottesville dataset have on average many more followers than the random sample, whose mean is `r round(mean(random_users$followers_count), 0)`. The difference between the two distributions is statistically significant (*p* = `r if (ks.test(random_users$followers_count, user_sample$followers_count)$p.value < 0.01) {0.01}`). The #Charlottesville public accordingly contains more highly visible tweeters than average.

[^4]:Collected by randomly streaming tweets via the twitter API, mid-afternoon on 08/06/2020.
...
``` {r at_assocs, fig.cap = "Figure 4: Words associated to accounts addressed with \\@"}
...
Figure 4 shows that \@realdonaldtrump and \@potus (‘President Of The United States’) are addressed in very similar terms: ‘nazi’ ‘attack’ ‘terrorist’ ‘hate’, ‘us’, ‘people’, ‘both sides’ are all used to address both as would be expected by a globally critical group of tweeters. \@gop (the Republican Party) and \@speakerryan [^5] are addressed in similar terms, by tweeters who exhort them to think of ‘party’, ‘country’, ‘support’, ‘moral’(ity) and the ‘kkk’: an attempt is being made to divide them from their president. ‘#impeachtrump’ and ‘nazi’ are associated with them as well as with Trump: tweeters are exhorting them to stand up to Trump by impeaching him. \@foxnews, \@cnn and \@seanhannity (a Fox News host, then as now a prominent Trump supporter) are associated, with ‘truth’ and ‘blame’, as the two networks are frequently contrasted by Trump and his opponents as, variously, paragons of impartial news and dishonest rumour-mongerers. Finally the left-wing media, \@nytimes, and \@vice, as well as \@youtube, like Vice News an internet platform, are associated in similar terms: ‘watch’, ‘tonight’, and – showing their counter-protest take on the issue – ‘race’.

[^5]: At the time, the most senior Republican politican apart from Trump and Vice-President Pence.
...
if (FALSE) {
  ...
  #model
  topic_mod = LDA(dtm, k = 6, control = list(seed = 234932, alpha = 0.05))
} else topic_mod = read_rds("../resources/new_topic_mod_very_low_alpha.rds.gz")

#get the topic a tweet is most likely to belong to
twts_by_topic = tidy(topic_mod, matrix = "gamma") %>%
  with_groups(document, ~ slice_max(., gamma, n = 1)) %>%
  filter(gamma > 0.5)

```
In an attempt to produce a good initial model, I have set the LDA parameter alpha very low, to 0.05, to reduce the number of topics per tweet as much as possible, as well as choosing a quite large number of topics, to find as many actual topics as possible at the cost of having some meaningless topics. The model was based on the most frequent 100 words, with hashtags, mentions, ‘via’ and ‘trump’ excluded. Figure 5 shows the results. From a statistical point of view, the model performs quite well as `r round(nrow(twts_by_topic) / n_distinct(tidy(topic_mod, matrix = "gamma")$document) * 100, 1)`% of the tweets have a probability > 0.5 of belonging to a topic, showing that the model is reasonably confident in its classification. But that does not in itself mean that the generated topics are meaningful to a reader.

``` {r topic_model_fig, fig.cap = "Figure 5: Associations of words to topics. Beta is the probability that a word is generated by a topic."}

#plot top word associations to topic
tidy(topic_mod, matrix = "beta") %>% #get word-topic probabilities
  left_join(count(twts_by_topic, topic), by = "topic") %>% #get n of twts for each topic
  mutate(label = str_glue("topic {topic}, n = {n}")) %>% #create facet titles
  with_groups(topic, ~ slice_max(., beta, n = 6)) %>% #get top 6 associations
  ggplot(aes(x = reorder_within(term, beta, label), y = beta, fill = label)) +
    geom_col() +
    ...
    labs(title = "Topic-word associations",
         caption = "Tweets classified in a topic if gamma > 0.5",
         x = "word", y = "beta") +
    theme_minimal() +
...
Topics 1 and 9 both link Trump’s press conference, his words (‘remarks’ in topic 9), and nazism. ‘just’ (topic 1) and ‘now’ (topic 9) show that these tweets are phrased as reactions to current events. Topic 3 also concerns Trump’s press conference, but it focuses less on the associations of Trump’s words to nazism than on the quote ‘on both sides’. Topics 2 and 7 are formed in reaction to two tweets that were copied multiple times: a share of a video from Vice News that was made with variations, `r tweets %>% filter(str_detect(full_text, "VICE News Tonight")) %>% nrow()` times; and tweets in very similar wording commemorating the success of Obama’s tweet about Charlottesville were made `r tweets %>% filter(str_detect(full_text, "(?i)Obama.*Charlottesville.*tweet")) %>% nrow()` times. Topic 4 is focused around the people, and their engagement to the topic, ‘love’, and ‘please’ ‘look’, while 6 is more interrogatory, most remarkable for collecting tweets that interpellate others with ‘u’ (you). Topic 8 gathers tweets that relate charlottesville authorities: the mayor and the police. Topic 5 has no pattern I can discern.

The topic model does therefore identify some groupings of significant actors and ways of presenting them. Trump’s press conference is seen in terms of a current event, or as evidence of fascism, or as particularly linked to his ‘both sides comment’. Some tweets focus mostly on the Charlottesville authorities and the local response. Others call on the rest of the community, whether to ‘please look’ and engage, or more aggressively via ‘you’. But on the other hand the topic model is distracted by certain very common tweets, and meaningful patterns in the actual tweets are often less than clear, especially in the case of topic 5. Many tweets need to be read to find the pattern in each topic, and many tweets are assigned a high probability of being in a given topic but on examination seem unrelated. The model requires much refinement, probably through a more careful selection of the word list.
...
In order to attempt to map the oppositions between social actors in the tweets, I constructed a dictionary that identifies identifies `r n_distinct(dict$assoc)` actors, politicians, and groupings via `r n_distinct(dict$word)` words.  The question then becomes how to determine who opposes who in a given tweet. One option is to use word sentiments. The tweets in the dataset are very characteristic of the polemic of twitter, a product of depersonalised online interaction where face-to-face – but not all – constraints on acceptable forms of speech have been lifted (Mercier 2015, Jehel 2018). However, the aggressiveness and negativity of the language is difficult to capture in a word-sentiment dictionary. 

``` {r sent_histogram, fig.width = 5, fig.asp = 5/9, fig.cap = "Figure 6: Histogram of tweets by sentiment"}
...
ggplot(tweets_sentiments, aes(x = twt_sentiment)) +
  geom_histogram(aes(y = ..count..), binwidth = 1, fill = "#7CBA7C") +
...
Figure 6 shows the distribution of tweets by their sentiment using the AFINN lexicon (Nielsen 2011) in which words are numerically coded, allowing a Figure for the tweet to be obtained by adding the codings of individual words. The distribution is negatively skewed, although the skew might be thought to not be very particularly strong given what is a set of very emotionally powerful texts. This suspicion that sentiment analysis is not adapted to this dataset is strengthened by an analysis of examples. The following groups of tweets each have a total sentiment of +5 and -5 respectively: while it is clear that the negative group do contain more negative words, the ‘positive’ group appear every bit as hostile and aggressive.
